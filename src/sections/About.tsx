...
export function About() {
  ...
  return (
    <section id="about" className="py-24 md:py-32 bg-white relative z-10 overflow-hidden">
      <div className="absolute top-0 right-0 w-[800px] h-[800px] bg-cyan/5 rounded-full blur-[120px] -translate-y-1/2 translate-x-1/3 pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          ...
          >
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-display font-semibold text-navy mb-8">
              {t.title} <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-navy to-gold">{t.titleHighlight}</span>
            </h2>
            <p className="text-lg text-navy/80 font-light leading-relaxed mb-8">
              {t.desc1}
            </p>
            <p className="text-lg text-navy/80 font-light leading-relaxed mb-8 md:mb-12">
              {t.desc2}
            </p>
              ...
              {t.values.map((value, i) => (
                ...
                >
                  <LiquidGlass intensity="light" className="px-6 py-3 rounded-full border-cyan/20">
                    <span className="text-sm font-medium tracking-wide text-navy uppercase">{value}</span>
                  </LiquidGlass>
                </motion.div>
              ))}
            </div>
            ...
            <div className="space-y-12">
              {t.timeline.map((item, index) => (
                ...
                >
                  ...
                  <p className="text-navy/80 font-light leading-relaxed">{item.desc}</p>
                </motion.div>
              ))}
            </div>
          </motion.div>
